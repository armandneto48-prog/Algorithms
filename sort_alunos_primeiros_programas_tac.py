...
def insertionSort(V):
    ...
    for j in range(1, N):
        key = V[j]
        i = j - 1
        # O erro estava aqui: o i -= 1 tem de estar alinhado dentro do while
        while i >= 0 and V[i] > key:
            V[i + 1] = V[i]
            i -= 1
        # O elemento chave é colocado na posição correta após o deslocamento
        V[i + 1] = key
...
# --- Código para testar no terminal ---
if __name__ == "__main__":
    minha_lista = [12, 34, 54, 2, 3, 1, 90, 45]
    # Gaps comuns (sequência de Shell): N/2, N/4...
    meus_gaps = [4, 2, 1] 
    
    print(f"Lista original: {minha_lista}")
    shellSort(minha_lista, meus_gaps)
